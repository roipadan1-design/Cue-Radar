...
implementation once reviewed. Also committed as `docs/design/VISUAL_IDENTITY_REFRESH_SPEC.md`
(the filename `docs/tasks/TASK_19_visual_identity_refresh.md` §1 and its verification block
actually check for) — identical content, two paths, so neither the task file's own `ls` check
nor this session's naming instruction fails.
